...
- **Training metrics & plots:** <https://huggingface.co/datasets/Tejasghatule/membrane-grpo-results> — checkpoints, CSVs, and job outputs on the Hub. **Figures to share:** **Files → `showcase/`** (SVGs + README). To refresh that folder from this repo: `HF_TOKEN=... python scripts/analysis/upload_showcase_to_hf_dataset.py` (separate from `git push`).
